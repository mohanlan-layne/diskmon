...
func buildLogger(logPath string) *slog.Logger {
	...
	// would silently stop writing the file too. newLogWriter wraps stdout so its
	// errors are swallowed and never block the file write.
	w := newLogWriter(os.Stdout, f)
